...
Multiverso - Equipa de desenvolvimento de software
...
## ⚙️ Como funciona
- O sistema liga-se ao MongoDB (MTLINKi)
- Liga-se ao broker MQTT (Mosquitto)
- Carrega o último _id processado (para evitar duplicados)
- A cada intervalo de tempo:
- Consulta novos registos nas coleções:
-- L1_Pool (tempos)
-- ProductResult_History (produção)
- Para cada novo registo:
- Converte o documento para JSON
- Publica no respetivo tópico MQTT:
-- tesco/tempos
-- tesco/producao
- Atualiza o último _id processado em ficheiro local
...
- Servidor com o software MTLINKi
- Servidor com Broker MQTT Mosquitto
