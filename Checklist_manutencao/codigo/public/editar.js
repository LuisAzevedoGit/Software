...
function renderizarChecklist() {
      ...
      <tr>
        ...
        <td>
          <select onchange="alterarStatus(${i}, this.value)">
            ...
            <option value="Não aplicado" ${acao.status === "Não aplicado" ? "selected" : ""}>Não aplicado</option>
          </select>
        </td>
  ...
  try {
    anexosExistentes = typeof checklistAtual.anexos === "string"
    ...
    <div id="lista-anexos">
      ...
      ${
        novosAnexos.length > 0
          ? novosAnexos.map((file, i) => `
              <div>
                📎 ${file.name}
                <button onclick="removerNovoAnexo(${i})">❌</button>
              </div>
            `).join('')
...
function adicionarAnexos(event) {
  ...
  files.forEach(f => {
    if (!novosAnexos.some(n => n.name === f.name && n.size === f.size)) {
      novosAnexos.push(f);
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (id) {
    document.getElementById("checklist_id").value = id;
    carregarChecklist();
  }
});